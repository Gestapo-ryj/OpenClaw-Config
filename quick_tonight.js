...
function parseMatches(text) {
  const matches = [];
  // 预处理：在每个时间前添加分隔符
  let processed = text.replace(/(\d{2}:\d{2})/g, '|||$1');
  // 分割
  ...
  for (const part of parts) {
    ...
    // 检查是否是日期行
    if (trimmed.match(/(Mon|Tue|Wed|Thu|Fri|Sat|Sun), \d+ (Jan|Feb|Mar|Apr|May|Jun|Jul|Aug|Sep|Oct|Nov|Dec) \d{4}/)) {
      currentDate = trimmed;
      continue;
    }
    ...
    if (timeMatch) {
      ...
      if (leagueMatch) {
        ...
        if (oddsMatch) {
          const parseChange = (str) => str ? parseFloat(str) : 0;
          
          matches.push({
            date: currentDate,
            time,
            league,
            homeWin: { odds: parseFloat(oddsMatch[1]), change: parseChange(oddsMatch[2]) },
            draw: { odds: parseFloat(oddsMatch[3]), change: parseChange(oddsMatch[4]) },
            awayWin: { odds: parseFloat(oddsMatch[5]), change: parseChange(oddsMatch[6]) }
          });
        }
      }
    }
...
// 筛选今晚比赛（19:09 - 04:00）
function filterTonightMatches(matches) {
  ...
  for (const match of matches) {
    ...
    if ((hour >= 19 && hour <= 23) || (hour >= 0 && hour <= 4)) {
      // 排除女足比赛
      if (match.league.includes('(Women)')) {
        continue;
      }
      tonightMatches.push(match);
    }
  }
  
  return tonightMatches;
}

// 分析变化
function analyzeChanges(matches, threshold = 10) {
  const significant = [];
  let maxIncrease = { value: 0, match: null, option: '' };
  let maxDecrease = { value: 0, match: null, option: '' };
  ...
  for (const match of matches) {
    ...
    ['homeWin', 'draw', 'awayWin'].forEach(option => {
      ...
      if (change >= threshold) {
        changes.push({ option, change: match[option].change, odds: match[option].odds });
        
        if (match[option].change > maxIncrease.value) {
          maxIncrease = { value: match[option].change, match, option };
        }
