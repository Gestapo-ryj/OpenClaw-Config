...
const { searchTeamId } = require('./search_team_fixed.js');
...
async function analyzeMatch(teamName, options = {}) {
  ...
  try {
    ...
    console.log(`   找到球队: ${teamInfo.name} (ID: ${teamInfo.id})`);
    
    // 步骤2: 获取球队赛程
    console.log('2. 获取球队赛程...');
    const schedule = await getTeamSchedule(teamInfo.id, { limit, timeRange });
    
    console.log(`   找到 ${schedule.matches.length} 场比赛`);
...
function formatReport(report, format = 'text') {
  ...
  output += `📱 数据来源: titan007.com (部分数据可能为模拟数据)\n`;
  output += `${'='.repeat(60)}\n`;
...
async function main() {
  ...
  if (args.length === 0) {
    ...
    console.log('用法: node match_analyzer_fixed.js "球队名称" [选项]');
    console.log('选项:');
