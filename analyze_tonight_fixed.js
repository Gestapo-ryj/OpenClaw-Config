...
class TonightOddsAnalyzer extends OddsAnalyzer {
  constructor(options = {}) {
    super(options);
    this.currentTime = options.currentTime || '00:51'; // 当前时间（新加坡时间00:51）
  ...
  extractFromSgoddsText(text) {
    ...
    for (let i = 0; i < lines.length; i++) {
      ...
      if (timeMatch) {
        ...
        // 提取联赛和赔率
        const leagueMatch = restOfLine.match(/^([A-Za-z].*?)(\d+\.\d+)/);
        if (leagueMatch) {
          currentLeague = leagueMatch[1].trim();
          
          // 提取赔率数据
          const oddsData = this.extractOddsFromLine(restOfLine.substring(leagueMatch[1].length));
          ...
          if (oddsData && oddsData.length >= 3) {
            ...
            // 判断是否今晚比赛（从今晚开始到明天凌晨4点）
            const isTonight = this.isTonightMatch(matchDate, now);
            ...
            if (isTonight) {
              matches.push({
                ...
                drawOdds: oddsData[1],
                awayOdds: oddsData[2],
                homeChange: oddsData[3] || '0%',
                drawChange: oddsData[4] || '0%',
                awayChange: oddsData[5] || '0%',
                matchTime: matchDate
              });
            }
          }
  ...
  // 判断是否今晚比赛
  isTonightMatch(matchDate, currentTime) {
